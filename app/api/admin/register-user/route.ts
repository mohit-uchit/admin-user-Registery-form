...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[v0] Hashing phone number for secure storage...")
    const hashedPhoneNumber = await bcrypt.hash(userData.mobileNo, 12)
    ...
    const newUser = new User({
      ...
      hashedPassword: hashedPhoneNumber, // Store hashed phone number instead of plain text
    })
    ...
    try {
      ...
      const emailHtml = getRegistrationEmailTemplate(userData.name, profileUrl)

      console.log("[v0] Preparing to send email to:", userData.emailId)
      console.log("[v0] Profile URL for email:", profileUrl)
